...
gmd(
  ...
  async (from, Gifted, conText) => {
    ...
    if (!imageUrl) {
      await react("❌");
      return reply(
        "Please provide an image URL or quote an image with a prompt\n\nUsage: .photoeditor <url> <prompt>\nOr quote an image with: .photoeditor <prompt>",
      );
    }
...
gmd(
  ...
  async (from, Gifted, conText) => {
    const { reply, react, q, botFooter, botName } = conText;
...
gmd(
  ...
  async (from, Gifted, conText) => {
    ...
    if (!binary) {
      await react("❌");
      return reply(
        "Please provide binary to convert\n\nUsage: .debinary 01001000 01100101 01101100 01101100 01101111",
      );
    }
